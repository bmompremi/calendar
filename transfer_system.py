"""
Money Transfer System for Haiti Banking
...
from config import (
    ...
    MIN_TRANSFER_AMOUNT, EXCHANGE_RATES, HAITI_BANKS
)
...
class TransferSystem:
    ...
    def transfer_domestic(self, from_account_number: str, to_account_number: str,
        ...
        to_account.balance += recipient_amount
        from_account.updated_at = datetime.now()
        to_account.updated_at = datetime.now()

        # Update daily transfer tracking
        from_account.daily_transfer_total += amount
        from_account.last_transfer_date = datetime.now()
    ...
    def get_transfer_history(self, account_number: str, days: int = 30) -> List[Transaction]:
        ...
        transactions = self.banking_system.get_account_transactions(account_number, limit=1000)

        transfer_types = ["TRANSFER_SENT", "TRANSFER_RECEIVED", "TRANSFER_INTERNATIONAL"]
        transfers = [
            t for t in transactions
            if t.transaction_type in transfer_types and t.timestamp >= cutoff_date
        ]
